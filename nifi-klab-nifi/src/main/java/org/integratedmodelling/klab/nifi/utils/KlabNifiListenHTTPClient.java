...
public class KlabNifiListenHTTPClient {
    ...
    public void submitObservation(KlabObservationNifiRequest req) throws Exception {
        ...
        HttpResponse<JsonNode> postResponse = Unirest.post(buildUrl(port, ""))
                .header("Accept", "application/json")
        ...
        if (!postResponse.isSuccess()) {
            throw new KlabNifiException(String.format(
                    "Failed to submit observation: HTTP %d - %s",
                    postResponse.getStatus(),
                    postResponse.getBody() != null ? postResponse.getBody().toString() : "No response body"
            ));
        }
